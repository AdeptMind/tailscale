...
func (m *resolvedManager) run(ctx context.Context) {
	...
	reconnect := func() error {
		...
		m.logf("[v1] Connecting DBUS or reconnecting to recover disconnect.")
		...
		// Reset backoff after successful on reconnect.
		bo.BackOff(ctx, nil)
		return nil
	}
...
func (m *resolvedManager) Close() error {
	...
	// No need to do anything on close which is handled in run.
	return nil
}
